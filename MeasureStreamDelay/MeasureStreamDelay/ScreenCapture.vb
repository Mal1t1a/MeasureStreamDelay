...
Namespace ScreenShot
    ...
    Public Class ScreenCapture
        ...
        Public Function CaptureDeskTopRectangle(ByVal CapRect As Rectangle, ByVal CapRectWidth As Integer, ByVal CapRectHeight As Integer) As Bitmap
            ...
            Dim SC As ScreenShot.ScreenCapture = FrmMeasure.SC
            Dim bmpImage As Bitmap 'New Bitmap(sc.CaptureScreen)

            Dim screenWidth As Integer = Convert.ToInt32(SystemParameters.VirtualScreenWidth)
            Dim screenHeight As Integer = Convert.ToInt32(SystemParameters.VirtualScreenHeight)
            Dim screenLeft As Integer = Convert.ToInt32(SystemParameters.VirtualScreenLeft)
            Dim screenTop As Integer = Convert.ToInt32(SystemParameters.VirtualScreenTop)
            Using bmp As New Bitmap(screenWidth, screenHeight, Drawing.Imaging.PixelFormat.Format32bppArgb)
                Using gr As Graphics = Graphics.FromImage(bmp)
                    gr.CopyFromScreen(screenLeft, screenTop, 0, 0, bmp.Size)
                    bmpImage = New Bitmap(bmp)
                    bmp.Dispose()
                    gr.Dispose()
                    'bmp.Save(Filename)
                End Using
